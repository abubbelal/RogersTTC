...
const TTCChart = () => {
    ...
    const loadData = async (dates = null) => {
        setError(false)
        setLoading(true)
        try {
            // const response = await axios.get(`${url}&selected_dates=${dates || last3Days().dates}`)
            // const data = response.data;
            // if(data.cols) {
            //     setChartData(data)
            // } else {
            //     throw "Data does not have any columns"
            // }
    ...
    const handleFetchData = async () => {
        ...
        const datesToFetch = selectedDates.map(date => date.toString()).toString();
        loadData(datesToFetch)
    }
    ...
    return (
        <>
            ...
            <div className="multi-date-picker flex flex-row gap-2 items-center px-30 border-b border-b-m-blue-200 pb-2">
                <DatePicker
                    multiple
                    maxDate={new Date()}
                    ...
                    format="YYYY-MM-DD"
                    plugins={
                        [<DatePanel header="Dates Selected" />]
                    }
                    onChange={handleDateChange}
                    inputClass=""
                    value={last3Days().calendarDates}
                />
                <button onClick={handleFetchData} className="bg-transparent text-m-black-600 py-1.5 px-3.5 border border-m-black-500 text-sm font-normal leading-none hover:bg-m-black-900 hover:text-m-black-100 transition-colors duration-150 ease-linear">Fetch Data</button>
            </div>
                            ...
                            <div className="chart-content h-full flex flex-row flex-grow">
                                <ChartTabs>
                                    <Tab title="TTC Charts">
                                        <ControlledCharts data={chartData} />
                                    </Tab>
